...
int grib_openjpeg_encode(grib_context* c, j2k_encode_helper* helper)
{
    ...
    // printf("novalues: %li, width: %li, height: %li\n", no_values, helper->width, helper->height);
    /* get a J2K compressor handle */
    ...
    // char* base = "/home/asaadco/github/panos/build/EncodedJPGs/";
    // char* ext = ".j2k";
    // snprintf(buffer, 200, "%s%i%s", base, fcount, ext);
    // const char* dump_jpg = buffer;
    // fcount++;
    // // if (dump_jpg) {
    // //     FILE* f = fopen(dump_jpg, "w");
    // //     if (f) {
...
int grib_openjpeg_encode_asaad(grib_context* c, j2k_encode_helper* helper) // my implementation
{
    // To-Do Implement the encoder according to the test of openjpeg
    int err = GRIB_SUCCESS;
    // Initialization of opj
    const char* v = opj_version();
    const OPJ_COLOR_SPACE color_space = OPJ_CLRSPC_GRAY;
    unsigned int numcomps = 1;

    opj_cparameters_t parameters = {0,};
    unsigned int subsampling_dx = 1; // Set to 1 by GRIB developers, changing this might have an impact on quality 
    unsigned int subsampling_dy = 1; // Set to 1 by GRIB developers, changing this might have an impact on quality 
    opj_image_cmptparm_t cmptparm = {0,};
    opj_image_t *image;
    ...
    // file saving
    char buffer[200];
    unsigned static int fcount = 1;
    char* base = "/home/asaadco/github/panos/build/EncodedJPGs_new/"; // create dir!
    char* ext = ".j2k";
    snprintf(buffer, 200, "%s%i%s", base, fcount, ext);
    fcount++;
    const char* outputfile = buffer;
    // check if file already exists (This was added to debug the issue of having more images than messages)
    ...
    double divisor         = helper->divisor;
    double decimal         = helper->decimal;
    /* initialize image encoder params */
    opj_set_default_encoder_parameters(&parameters);
    parameters.cod_format = 0; // J2K -> 0, JP2 -> 1
    parameters.tcp_numlayers  = 1;
    parameters.cp_disto_alloc = 1;
    parameters.tcp_rates[0] = helper->compression;
    ...
    cmptparm.prec = helper->bits_per_value; // this is the new standard. do not use bpp. you can also manually set this to 8 bits per pixel
    cmptparm.sgnd = 0;
    ...
    image->x1 = helper->width;
    image->y1 = helper->height;
    // Simple data packing.... there might be other methods
    data = image->comps[0].data;
    ...
    // callbacks for the compressor
    opj_set_info_handler(l_codec, openjpeg_info, c);
    opj_set_warning_handler(l_codec, openjpeg_warning, c);
    opj_set_error_handler(l_codec, openjpeg_error, c);
...
int grib_openjpeg_decode_old(grib_context* c, unsigned char* buf, size_t* buflen, double* val, size_t* n_vals) // my implementation
{
    ...
    char buffer[400];
    unsigned static int fcount = 1;
    char* base = "/home/asaadco/github/panos/build/decodedJPGs_Temp100/"; // create dir!
    char* ext = ".jp2";
    snprintf(buffer, 400, "%s%i%s", base, fcount, ext);
    fcount++;
    const char* outputfile = buffer;

    l_stream = opj_stream_create_default_file_stream(outputfile, 1);
    grib_context_log(c, GRIB_LOG_ERROR, "Reading from: %s", outputfile);

    d_codec = opj_create_decompress(OPJ_CODEC_JP2);
    opj_set_info_handler(d_codec, openjpeg_info, c);
    opj_set_warning_handler(d_codec, openjpeg_warning, c);
    opj_set_error_handler(d_codec, openjpeg_error, c);
    ...
    if (!(*n_vals <= image->comps[0].w * image->comps[0].h)) {
        grib_context_log(c, GRIB_LOG_ERROR, "openjpeg: failed to decode meow");
        err = GRIB_DECODING_ERROR;
        goto cleanup;
    }
...
int grib_openjpeg_decode(grib_context* c, unsigned char* buf, size_t* buflen, double* val, size_t* n_vals)
{
    ...
    // grib_context_log(c, GRIB_LOG_ERROR, "What is number of components %d", image->numcomps);
    // grib_context_log(c, GRIB_LOG_ERROR, "x1: %d and y1: %d", image->x1, image->y1);
    if ((image->numcomps != 1) || (image->x1 * image->y1) == 0) {
